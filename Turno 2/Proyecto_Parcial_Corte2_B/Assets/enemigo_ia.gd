...
@export var rango_vision: float = 10.0
...
func _tiene_linea_vision() -> bool:
	...
	if vision.is_colliding():
		var colisionador = vision.get_collider()
		return colisionador == jugador or (colisionador is Node and colisionador.is_in_group("player"))
	return false
...
func _moverse_lejos_de(amenaza: Vector3, delta: float) -> void:
	# Huir = perseguir con la dirección invertida. Nada más cambia.
